...
static esp_err_t panel_sh8601_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
                                          int y_start, int x_end, int y_end,
                                          const void *color_data);
static esp_err_t panel_sh8601_invert_color(esp_lcd_panel_t *panel,
                                           bool invert_color_data);
static esp_err_t panel_sh8601_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                     bool mirror_y);
static esp_err_t panel_sh8601_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
static esp_err_t panel_sh8601_set_gap(esp_lcd_panel_t *panel, int x_gap,
                                      int y_gap);
...
esp_err_t
esp_lcd_new_panel_sh8601(const esp_lcd_panel_io_handle_t io,
  ...
  ESP_GOTO_ON_FALSE(sh8601, ESP_ERR_NO_MEM, err, TAG,
                    "no mem for sh8601 panel");
  ...
  sh8601_vendor_config_t *vendor_config =
      (sh8601_vendor_config_t *)panel_dev_config->vendor_config;
  if (vendor_config) {
...
static esp_err_t panel_sh8601_reset(esp_lcd_panel_t *panel) {
  ...
  if (sh8601->reset_gpio_num >= 0) {
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_SWRESET, NULL, 0), TAG,
                        "send command failed");
...
static esp_err_t panel_sh8601_init(esp_lcd_panel_t *panel) {
  ...
  if (sh8601->init_cmds) {
    ...
    init_cmds_size =
        sizeof(vendor_specific_init_default) / sizeof(sh8601_lcd_init_cmd_t);
  }
...
static esp_err_t panel_sh8601_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
  ...
  ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_RASET,
                      ...
                      TAG, "send command failed");
  size_t len =
      (x_end - x_start) * (y_end - y_start) * sh8601->fb_bits_per_pixel / 8;
...
static esp_err_t panel_sh8601_invert_color(esp_lcd_panel_t *panel,
  ...
  ESP_RETURN_ON_ERROR(tx_param(sh8601, io, command, NULL, 0), TAG,
                      "send command failed");
  return ESP_OK;
}
...
static esp_err_t panel_sh8601_mirror(esp_lcd_panel_t *panel, bool mirror_x,
  ...
  ESP_RETURN_ON_ERROR(
      tx_param(sh8601, io, LCD_CMD_MADCTL, (uint8_t[]){sh8601->madctl_val}, 1),
      TAG, "send command failed");
  return ret;
}
