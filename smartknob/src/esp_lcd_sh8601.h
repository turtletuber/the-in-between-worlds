 ...
 *
 * @note  This structure can be used to select interface type and override
 ...
 */
typedef struct {
  const sh8601_lcd_init_cmd_t
      *init_cmds;          /*!< Pointer to initialization commands array.
                            *  The array should be declared as `static const` and
                            * positioned outside the function.          Please refer to
                            * `vendor_specific_init_default` in source file
                            */
 ...
 * @param[in]  panel_dev_config General panel device configuration (Use
 * `vendor_config` to select QSPI interface or override default initialization
 * commands)
